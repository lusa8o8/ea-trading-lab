//+------------------------------------------------------------------+
//| Tri Pair Optimized [Dynamic Risk] v3.0 — ADX Filter Variant     |
//| Full Strategy + CSV Trade Logging (36 Fields)                   |
//| Variant: adds UseADXFilter / ADXThreshold gate in CheckForOpen  |
//+------------------------------------------------------------------+
...
void CheckForOpen()
{
   ...
   //--- ADX filter: skip entry if trend strength is below threshold
   if(UseADXFilter)
     {
      double adx_buf[1];
      if(CopyBuffer(adxHandle, 0, 0, 1, adx_buf) != 1) return;
      if(adx_buf[0] < ADXThreshold) return;
     }
