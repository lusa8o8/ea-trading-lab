//+------------------------------------------------------------------+
//| Tri Pair Optimized [Dynamic Risk] v3.0 — Trail A Variant        |
//| Full Strategy + CSV Trade Logging (36 Fields)                   |
//| Variant: breakeven trailing stop based on MFE % of TP distance  |
//+------------------------------------------------------------------+
...
input double MAEGuardPct         = 85.0;
input double MAEGuardTighten     = 60.0;
input bool   UseTrailA           = false;
input double TrailA_ActivatePct  = 60.0;  // % of TP distance to activate
...
void LogTradeToCSV(ulong deal_ticket)
{
   ...
   //--- Override close reason to TRAIL_BE if trail A fired and exit is within 3 pips of entry
   if(g_trail_a_applied && MathAbs(exit_price - g_entry_price) / pip <= 3.0)
      close_reason = "TRAIL_BE";
   ...
   if(position_open)
     {
      ...
      //--- Trail A: move SL to breakeven when MFE reaches activation threshold
      if(UseTrailA && !g_trail_a_applied && g_tp_price != 0 && g_entry_price != 0)
        {
         double pip          = PipSize();
         double tp_dist_pips = MathAbs(g_tp_price - g_entry_price) / pip;

         if(g_mfe_pips >= (TrailA_ActivatePct / 100.0) * tp_dist_pips)
           {
            ulong ticket = (ulong)PositionGetInteger(POSITION_TICKET);
            if(ExtTrade.PositionModify(ticket, g_entry_price, g_tp_price))
              {
               g_trail_a_applied = true;
               Print("Trail A activated — SL moved to breakeven | ticket:", ticket);
              }
           }
        }
